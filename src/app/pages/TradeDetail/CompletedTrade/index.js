...
import {
  Text, View, Image, TextInput, TouchableHighlight, AsyncStorage, FlatList, ActivityIndicator
} from 'react-native';
...
export default class CompletedTrade extends Component {
  ...
  stopLoading(){
     ...
     getCompletedTrade=async(pageNumber)=>{
       if(pageNumber==1)
         ...
         await WebApi.postApi_trade('getCompleteTradeList', body)
          .then(response => response.json())
            .then(json => {
               this.setState({refreshing:false, loadingMore:false});
                 console.log('Response from CompletedTrades===>', json);
  ...
  render() {
    ...
    return (
      <View style={Styles.body}>
       <Header title='Completed Trades' menuCheck="false" data={this.props} style={Styles.header}/>
        <ProgressBar
          title="Please Wait"
        ...
        />
          <View style={Styles.container}>
                  ...
                  <FlatList
                    ...
                    onRefresh={()=>this.getCompletedTrade(1)}
